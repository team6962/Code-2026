...
public class AprilTagCamera {
  ...
  /** The PhotonPoseEstimator used to estimate the robot's pose based on AprilTag detections. */
  ...
  /**
   * Constructs an AprilTagCamera with the specified parameters. The constructed object can then be
   * used to obtain robot pose estimates based on AprilTag detections via {@link
   * #getRobotPoseEstimates()}.
   *
   * @param cameraConstants Configuration constants for this camera
   * @param visionConstants Configuration constants for AprilTag vision processing
   * @param visionSystemSim VisionSystemSim for adding the camera to simulation (or null if not in
   *     simulation)
   */
  public AprilTagCamera(
      AprilTagCameraConstants cameraConstants,
      AprilTagVisionConstants visionConstants,
  ...
  /**
   * Gets the latest robot pose estimates from the camera's pose estimator. Note that this should be
   * called exactly once per control loop cycle to ensure that all new results are processed and no
   * old results are used.
   *
  ...
  public List<AprilTagVisionMeasurement> getRobotPoseEstimates() {
    ...
    for (int i = results.size() - 1; i >= 0; i--) {
      ...
      Optional<Matrix<N4, N1>> possibleStdDevs =
          getStandardDeviations(estimate, result.getTargets());
      ...
      DogLog.log(
          "Vision/Cameras/" + device.getName() + "/Estimates/" + (estimateCount - 1) + "/Pose",
          estimate.estimatedPose);
      DogLog.log(
          "Vision/Cameras/" + device.getName() + "/Estimates/" + (estimateCount - 1) + "/Latency",
          Timer.getFPGATimestamp() - estimate.timestampSeconds);
      DogLog.log(
          "Vision/Cameras/" + device.getName() + "/Estimates/" + (estimateCount - 1) + "/Strategy",
          estimate.strategy);
      DogLog.log(
          "Vision/Cameras/" + device.getName() + "/Estimates/" + (estimateCount - 1) + "/TagCount",
   ...
   *
   * <p>This code was adapted from the <a
   * href="https://github.com/PhotonVision/photonvision/blob/main/photonlib-java-examples/poseest/src/main/java/frc/robot/Vision.java#L125-L165">
   * <code>updateEstimationStdDevs</code></a> method in PhotonLib's example code.
   *
   * @param estimatedPose The estimated pose to guess standard deviations for
   * @param targets All targets in this camera frame
   * @return The calculated standard deviations
   */
  private Optional<Matrix<N4, N1>> getStandardDeviations(
    ...
    // that is rare.
    Matrix<N4, N1> estStdDevs =
        tagCount > 1 ? visionConstants.MultiTagStdDevs : visionConstants.SingleTagStdDevs;

    // Increase std devs based on average distance
    estStdDevs =
        estStdDevs.times(
            1 + (averageTagDistance * averageTagDistance) * visionConstants.StdDevDistanceScalar);
