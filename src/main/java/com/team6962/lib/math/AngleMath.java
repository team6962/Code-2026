...
public class AngleMath {
     ...
     * to a discrete angle, which is wrapped around to restrict it to the range
     * -180° to 180°.
     * 
    ...
    public static Angle toDiscrete(Angle continuousAngle) {
        ...
        double discreteRadians = continuousRadians - Math.floor(continuousRadians / (2 * Math.PI)) * (2 * Math.PI);
    ...
    /**
     * Converts a discrete angle (which is wrapped to the range -180° to 180°)
     * to a continuous angle, which can go past 360° to 361° and beyond. Given
     * a discrete angle, there are infinite possible equivalent continuous
     * angles, which are each 360° apart, so this method requires a nearby
     * continuous angle to determine which continuous angle to return. The
     * returned continuous angle will be the one that is closest to the provided
     * nearby continuous angle.
     * 
     * @param discreteAngle The discrete angle to convert
     * @param nearbyContinuousAngle A nearby continuous angle used to determine
     * which possible output to return
     * @return The equivalent continuous angle closest to the provided nearby
     * continuous angle
     */
