...
/**
 * A control loop abstraction that provides periodic execution of an update
 * function at a configurable frequency.
 *
 * <p>This interface allows different control loop implementations to be
 * swapped without changing the calling code. The update function receives
 * the actual delta time since the last update, enabling time-based
 * calculations that remain accurate even with timing jitter.
 *
...
public interface ControlLoop extends AutoCloseable {
    ...
    /**
     * A control loop implementation that runs on the main robot thread by
     * extending {@link SubsystemBase} and using its periodic method.
     *
     * <p>This implementation only supports 50 Hz (the robot's main loop
     * frequency). Other frequencies are ignored with a warning. Use
     * {@link Threaded} if a different frequency is required.
     */
    ...
    /**
     * A control loop implementation that runs on a dedicated thread using
     * WPILib's {@link Notifier}.
     *
     * <p>This implementation supports any update frequency and automatically
     * handles timing drift by tracking a goal timestamp. If execution falls
     * behind schedule, it will skip periods to catch up rather than queuing
     * multiple rapid updates.
     *
     * <p>The thread runs at real-time priority for consistent timing. Call
     * {@link #close()} when done to release the thread resources.
     */
