 ...
 * field-oriented control with configurable speed scaling, boost functionality, and fine control
 * mode using the D-pad.
 *
 * <p><b>Default Control Scheme:</b>
 *
 * <ul>
 *   <li>Left stick: Translation (field-oriented)
 *   <li>Right stick horizontal axis: Rotation
 *   <li>Right trigger: Boost (increases speed)
 *   <li>Left trigger: Angular super boost (maximizes rotation speed), designed to dislodge stuck
 *       game pieces
 *   <li>D-pad: Fine control mode (robot-relative slow movement)
 * </ul>
 *
 * <p>All control parameters are configured through {@link XBoxTeleopSwerveConstants}, including
 * joystick deadband, sensitivity curve, speed scaling factors, and button/trigger assignments.
 */
public class XBoxTeleopSwerveCommand extends TeleopSwerveCommand {
   ...
   * @param constants The configuration constants for controller mapping and speed settings
   */
  public XBoxTeleopSwerveCommand(
      CommandSwerveDrive swerveDrive, XBoxTeleopSwerveConstants constants) {
    super(swerveDrive);
    ...
    if (constants.ZeroYawButton != null) {
      ...
      commandController
          .button(constants.ZeroYawButton.value)
          .whileTrue(
  ...
  private ChassisSpeeds outputPowerToVelocity(ChassisSpeeds fractionMaxSpeeds) {
    double maxLinearVelocity =
        getSwerveDrive().getConstants().Driving.MaxLinearVelocity.in(MetersPerSecond);
  ...
  /**
   * Reorients the control inputs when running in simulation mode, if this feature is enabled. This
   * rotates the controls so that "up" on the joystick or keyboard moves the robot toward the top of
   * the screen rather than away from the driver station.
   *
   * @param speeds The original chassis speeds
   * @return The reoriented chassis speeds (unchanged if not in simulation or reorientation is
   *     disabled)
   */
  ...
  /**
   * Computes the desired field-relative velocity based on the current controller inputs. This
   * method handles both normal driving mode and fine control mode (when D-pad is pressed).
   *
  ...
  /**
   * Gets the translation speed scalar for normal (non-fine-control) mode, interpolated between
   * default and boost speeds based on boost input.
   *
  ...
  /**
   * Gets the angular speed scalar for normal (non-fine-control) mode, interpolated based on boost
   * and super boost inputs.
   *
  ...
  private Translation2d getMappedTranslationInput() {
    Translation2d rawInput =
        new Translation2d(
            -(constants.TranslationJoystick == Joystick.LeftStick
                ? controller.getLeftY()
                : controller.getRightY()), // Axis 1 or 5
            -(constants.TranslationJoystick == Joystick.LeftStick
                ? controller.getLeftX()
                : controller.getRightX()) // Axis 0 or 4
            );
  ...
  /**
   * Gets the mapped rotation input from the horizontal axis of the configured rotation joystick,
   * with deadband and sensitivity curve applied.
   *
   * @return The mapped rotation input (-1.0 to 1.0)
   */
  private double getMappedRotationInput() {
    double rawInput =
        constants.RotationJoystick == Joystick.LeftStick
            ? controller.getLeftX()
            : controller.getRightX(); // Axis 0 or 4
  ...
  /**
   * Maps a 2D joystick input through the deadband and sensitivity curve. The mapping is applied to
   * the magnitude of the input vector while preserving its direction.
   *
  ...
  /**
   * Maps a 1D joystick input through the deadband and sensitivity curve. Inputs below the deadband
   * return 0, and inputs above are scaled from the deadband threshold to 1.0, then raised to the
   * configured exponent.
   *
  ...
  /**
   * Checks if fine control mode is active. Fine control is activated when any D-pad direction is
   * pressed.
   *
  ...
  /**
   * Gets the fine control input direction from the D-pad. Returns a unit vector in the direction of
   * the pressed D-pad button.
   *
   * @return A unit vector in the D-pad direction, or zero if not fine controlling
   */
