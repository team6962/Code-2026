...
/**
 * Provides an interface to the Pigeon 2.0 gyroscope with methods to get latency-compensated
...
public class Gyroscope implements SwerveComponent, AutoCloseable {
  ...
  /**
   * The Pigeon 2.0 gyroscope instance. This communicates with the physical gyro hardware over CAN
   * bus.
   */
  private final Pigeon2 gyro;

  /**
   * The odometry object that provides the most recently executed twist. This is used to update the
   * gyroscope orientation when the gyroscope is disconnected or disabled.
   */
   ...
   *
   * @param constants The drivetrain constants containing gyroscope configuration
   */
  ...
  public BaseStatusSignal[] getStatusSignals() {
    return new BaseStatusSignal[] {
      yawSignal, yawVelocitySignal, pitchSignal, pitchVelocitySignal, rollSignal, rollVelocitySignal
    };
  }
  ...
  /**
   * Returns true if the real gyroscope data should be used, and false if odometry-derived data
  ...
  /**
   * Updates the cached gyroscope data. This method should not be called while the status signals
   * are being refreshed in a different thread.
   */
  @Override
  public synchronized void update(double deltaTimeSeconds) {
    if (!shouldUseGyroscope()) {
      ...
      if (constants.Gyroscope.LatencyCompensation) {
        yaw =
            Degrees.of(
                BaseStatusSignal.getLatencyCompensatedValueAsDouble(yawSignal, yawVelocitySignal));
