...
/**
 * Odometry class for tracking the positions and states of swerve modules and the changes in
 * position of the robot over time.
 */
public class Odometry implements SwerveComponent {
  /** Kinematics object for converting between module states/positions and robot motion. */
   ...
   * @param modules An array of SwerveModule objects representing the swerve modules being tracked.
   */
  ...
  /**
   * Computes the current positions (steer angles and drive positions) of all swerve modules. This
   * method should only be called internally to refresh the position data.
   *
   * @return An array of SwerveModulePosition objects representing the current positions of the
   *     swerve modules.
   */
  ...
  /**
   * Gets the current positions of all swerve modules. These include the distances each wheel has
   * traveled along the ground and the current direction that each wheel is facing.
   *
   * @return An array of SwerveModulePosition objects representing the current positions of the
   *     swerve modules.
   */
  ...
  /**
   * Computes the current states (steer angles and drive velocities) of all swerve modules. This
   * method should only be called internally to refresh the state data.
   *
  ...
  /**
   * Gets the current states of all swerve modules. These include the linear velocities and steer
   * angles of each wheel.
   *
   * @return An array of SwerveModuleState objects representing the current states of the swerve
   *     modules.
   */
  ...
  /**
   * Computes the change in position of all swerve modules since the last update. This method should
   * only be called internally to refresh the position delta data.
   *
   * @return An array of SwerveModulePosition objects representing the change in position of the
   *     swerve modules.
   */
  ...
  /**
   * Computes the twist the robot has undergone since the last update. This method should only be
   * called internally to refresh the twist data.
   *
   * @return A Twist2d object representing the change in position and orientation of the robot.
   */
  ...
  public synchronized void logTelemetry(String basePath) {
    ...
    for (int i = 0; i < modules.length; i++) {
      DogLog.log(
          basePath + "Module Positions/" + i + "/distanceMeters",
          currentPositions[i].distanceMeters,
          Meters);
      DogLog.log(
          basePath + "Module Positions/" + i + "/angleRadians",
          currentPositions[i].angle.getRadians(),
          Radians);
      DogLog.log(
          basePath + "Module States/" + i + "/speedMetersPerSecond",
          currentStates[i].speedMetersPerSecond,
          MetersPerSecond);
      DogLog.log(
          basePath + "Module States/" + i + "/angleRadians",
          currentStates[i].angle.getRadians(),
          Radians);
      DogLog.log(
          basePath + "Position Deltas/" + i + "/distanceMeters",
          positionDeltas[i].distanceMeters,
          Meters);
      DogLog.log(
          basePath + "Position Deltas/" + i + "/angleRadians",
          positionDeltas[i].angle.getRadians(),
          Radians);
    }
