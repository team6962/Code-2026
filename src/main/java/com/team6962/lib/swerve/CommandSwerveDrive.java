...
/**
 * A swerve drive class that provides command-based control of the rotational
 * and translational motion of the drivetrain. Note that this class does not
 * extend {@link SubsystemBase}, so commands do not run on a CommandSwerveDrive
 * directly. Instead, there are two seperate subsystems for controlling
 * translational and rotational motion, accessible via {@link #useTranslation()}
 * and {@link #useRotation()}, respectively.
 */
public class CommandSwerveDrive extends MotionSwerveDrive {
    ...
    /**
     * Gets the subsystem used for controlling translational motion. Commands
     * that need to control translation should require this subsystem.
     *
    ...
    /**
     * Gets the subsystem used for controlling rotational motion. Commands that
     * need to control rotation should require this subsystem.
     *
    ...
    /**
     * Gets both the translation and rotation subsystems as an array. Commands
     * that need to control both translation and rotation should require both
     * of these subsystems.
     *
    ...
    /**
     * Gets both the translation and rotation subsystems as a set. This is
     * useful for commands that accept a set of subsystem requirements.
     *
    ...
    /**
     * Creates a command that runs the given action repeatedly while requiring
     * only the translation subsystem.
     *
    ...
    /**
     * Creates a command that runs the given action repeatedly while requiring
     * both the translation and rotation subsystems.
     *
     * @param toRun The action to run each iteration
    ...
    /**
     * Creates a command that drives the robot at the velocity provided by the
     * supplier. The command requires both translation and rotation subsystems.
     *
    ...
    /**
     * Creates a command that drives the robot's translation at the velocity
    ...
    /**
     * Creates a command that applies a swerve motion, with an update function
     * called each iteration to modify the motion.
     *
    ...
    /**
     * Creates a command that puts all motors in the configured default neutral
     * mode.
     *
    ...
    /**
     * Creates a command that puts all motors in brake mode, actively resisting
     * motion.
     *
    ...
    /**
     * Creates a command that puts all motors in coast mode, allowing free
     * spinning.
     *
    ...
    /**
     * Creates a command that locks the wheels in an X pattern to prevent the
    ...
    /**
     * Creates a command that drives the robot to the specified target state
     * using motion profiling and PID.
     *
    ...
    /**
     * Creates a command that drives the robot to the specified pose with the
     * specified ending velocity.
     *
    ...
    /**
     * Creates a command that drives the robot to the specified pose, stopping
     * at the target.
     *
     * @param pose The target pose (position and heading)
     * @return A DriveToStateCommand configured with the target
    ...
    /**
     * Creates a command that drives the robot to the specified translation with
     * the specified ending translational velocity. Rotation is not controlled.
     *
    ...
    /**
     * Creates a command that drives the robot to the specified translation,
     * stopping at the target. Rotation is not controlled.
     *
     * @param translation The target translation (x, y position)
     * @return A DriveToStateCommand configured with the target
     */
    ...
    /**
     * Creates a command that rotates the robot to the specified heading,
     * stopping at the target. Translation is not controlled.
     *
     * @param angle The target heading angle
     * @return A DriveToStateCommand configured with the target
     */
    ...
    /**
     * Should be called at the end of each periodic cycle to clear the previous
     * motion and begin executing the most recently applied motion.
     */
