 ...
 *
 * <p>NeutralMotion disables active control of the swerve modules and allows them to either coast
 * freely or actively brake, depending on the configured neutral mode.
 *
 * <p>The neutral mode can be configured to:
 *
 * <ul>
 *   <li>{@link NeutralModeValue#Coast} - Motors spin freely with no resistance
 *   <li>{@link NeutralModeValue#Brake} - Motors actively resist rotation
 *   <li>{@code null} - Uses the default neutral output behavior specified in the motor controller
 *       configurations
 * </ul>
 */
public class NeutralMotion implements SwerveMotion {
  ...
  public NeutralMotion(MotionSwerveDrive swerveDrive, NeutralModeValue neutralMode) {
    ...
    if (neutralMode == NeutralModeValue.Coast) {
      request = new CoastOut().withUpdateFreqHz(updateFreqHz).withUseTimesync(useTimesync);
    } else if (neutralMode == NeutralModeValue.Brake) {
      request = new StaticBrake().withUpdateFreqHz(updateFreqHz).withUseTimesync(useTimesync);
    } else {
      request = new NeutralOut().withUpdateFreqHz(updateFreqHz).withUseTimesync(useTimesync);
    }
  }
