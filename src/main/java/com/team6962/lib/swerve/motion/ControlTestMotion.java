 ...
 *
 * <p>ControlTestMotion allows direct, low-level control of individual swerve module motors by
 * sending arbitrary {@link ControlRequest} objects to specific drive or steer motors. This is
 * primarily useful for:
 *
 * <ul>
 *   <li>Testing and tuning individual motor controllers
 *   <li>Debugging motor behavior in isolation
 *   <li>Characterization routines (SysId, etc.)
 *   <li>Manual motor control during development
 * </ul>
 *
 * <p>The class provides a builder API with {@code with*()} methods for convenient configuration, as
 * well as direct setter methods. Control requests are stored in an array where even indices (0, 2,
 * 4, 6) are drive motors and odd indices (1, 3, 5, 7) are steer motors.
 *
 * @see SwerveMotion
 * @see ControlRequest
 */
public class ControlTestMotion implements SwerveMotion {
  /**
   * Array of control requests: [ front left drive, front left steer, front right drive, front right
   * steer, back left drive, back left steer, back right drive, back right steer].
   */
   ...
   * @param steerRequest The control request for the steer motor
   */
  public synchronized void setModuleControl(
      Corner moduleCorner, ControlRequest driveRequest, ControlRequest steerRequest) {
    int moduleIndex = moduleCorner.getIndex();
    controlRequests[2 * moduleIndex] = driveRequest;
    controlRequests[2 * moduleIndex + 1] = steerRequest;
  }
   ...
   * @return This ControlTestMotion for method chaining
   */
  public ControlTestMotion withModuleControl(
      Corner moduleCorner, ControlRequest driveRequest, ControlRequest steerRequest) {
   ...
   * @return This ControlTestMotion for method chaining
   */
  public ControlTestMotion withAllModuleControl(
      ControlRequest driveRequest, ControlRequest steerRequest) {
