 ...
 * 
 * <p>A default motion must be provided at construction, which is returned by
 * {@link #getActiveMotion()} when no other motion is active.
 * 
 * @see SwerveMotion
 */
public class SwerveMotionManager implements AutoCloseable {
     ...
     * 
     * <p>If a motion is already queued, the two motions are fused together
     * using {@link SwerveMotion#fuseWith(SwerveMotion)}.
     * 
     * @param newMotion The motion to apply, or {@code null} to apply the
     *                  default motion
     */
