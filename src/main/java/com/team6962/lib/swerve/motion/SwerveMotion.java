...
/**
 * Interface representing a motion that a swerve drive can perform during a
 * single control loop iteration.
 * 
 * <p>A SwerveMotion encapsulates the logic for controlling swerve module states
 * to achieve a specific type of movement. Each iteration, the
 * {@link #update(double)} method is called to apply the motion's control
 * outputs to the swerve modules.
 * 
 ...
 *   <li>{@link TwistToTargetMotion} - Drives along a twist path to a target
 *       pose</li>
 *   <li>{@link ControlTestMotion} - Drives along a twist path to a target pose
 *       </li>
 * </ul>
 * 
 * <p>Motions can be fused together using {@link #fuseWith(SwerveMotion)} to
 * combine compatible motion types (e.g., combining translation-only and
 * rotation-only velocity motions).
 * 
 * <p>This interface extends {@link AutoCloseable} to allow cleanup of resources
 * when a motion is replaced or the swerve drive is closed.
 * 
...
public interface SwerveMotion extends AutoCloseable {
     ...
     * 
     * @param basePath The base path in the telemetry hierarchy for this
     *                 motion's data
     */
     ...
     * 
     * <p>Fusion allows combining compatible motions, such as merging a
     * translation-only velocity motion with a rotation-only velocity motion
     * into a single combined motion.
     * 
     * <p>The default implementation throws an exception, indicating that fusion
     * is not supported. Subclasses that support fusion should override this
     * method.
     * 
