 ...
 *
 * <p>TwistToTargetMotion creates a twist (circular arc) between the current pose and target pose,
 * which is followed to drive the robot to the target. The dynamic position control ensures smooth
 * acceleration and deceleration. This motion is great for precise autonomous positioning, but it is
 * not designed to maximize speed.
 *
 * @see SwerveMotion
 * @see VelocityMotion
 */
public class TwistToTargetMotion implements SwerveMotion {
  ...
  public void update(double deltaTimeSeconds) {
    ...
    SwerveModuleState[] positionDeltasAsStates =
        swerveDrive
            .getKinematics()
            .toSwerveModuleStates(new ChassisSpeeds(twist.dx, twist.dy, twist.dtheta));
    ...
    for (int i = 0; i < moduleCount; i++) {
      positionDeltas[i] =
          new SwerveModulePosition(
              positionDeltasAsStates[i].speedMetersPerSecond, positionDeltasAsStates[i].angle);
    ...
    for (int i = 0; i < moduleCount; i++) {
      ...
      positionDeltas[i] =
          SwerveKinematicsUtil.optimizeRelativePosition(positionDeltas[i], steerAngle);
      ...
      double cosineError =
          SwerveKinematicsUtil.getCosineOfSteerError(positionDeltas[i], steerAngle);
