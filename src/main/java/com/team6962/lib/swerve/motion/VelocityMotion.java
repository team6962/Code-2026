 ...
 *
 * <p>VelocityMotion converts field-relative chassis speeds into individual swerve module states and
 * applies velocity control to the drive motors and position control to the steer motors. Key
 * features include:
 *
 * <ul>
 *   <li>Field-relative velocity control (velocities are relative to the field, not the robot)
 *   <li>Automatic wheel speed desaturation to respect motor limits
 *   <li>Steer angle optimization to minimize wheel rotation
 *   <li>Velocity reduction when wheels are misaligned
 *   <li>Automatic braking when commanded velocity is near zero
 * </ul>
 *
 * <p>VelocityMotion supports fusion with other VelocityMotions that have non-overlapping
 * components. For example, a translation-only motion can be fused with a rotation-only motion to
 * create a combined motion with both components.
 */
public class VelocityMotion implements SwerveMotion {
   ...
   *
   * <p>Fusion is only allowed when the motions have non-overlapping components (e.g., one has
   * translation only and the other has rotation only).
   *
  ...
  public SwerveMotion fuseWith(SwerveMotion other) {
    if (other instanceof VelocityMotion otherVelocityMotion) {
      if (hasTranslation && otherVelocityMotion.hasTranslation
  ...
  public void update(double deltaTimeSeconds) {
    ...
    if (!swerveDrive.getConstants().Driving.MaxLinearVelocity.isEquivalent(MetersPerSecond.of(0))
        && !swerveDrive
            ...
            .Driving
            .MaxAngularVelocity
            .isEquivalent(RadiansPerSecond.of(0))) {
      ...
      SwerveDriveKinematics.desaturateWheelSpeeds(
          states, swerveDrive.getConstants().DriveMotor.MaxVelocity);
    }
