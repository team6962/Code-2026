 ...
 *
 * <p>LockMotion orients the swerve modules into an X configuration where each wheel points toward
 * or away from the center of the robot. This configuration makes it very difficult to push the
 * robot in any direction, which is when the robot is being defended to prevent opponents from
 * pushing the robot.
 *
 * <p>The lock pattern uses position control on both drive and steer motors to actively hold the
 * wheels at their target positions. The target positions are captured when the motion first starts,
 * with steer angles set to form the X pattern.
 */
public class LockMotion implements SwerveMotion {
   ...
   *
   * <p>On the first update, this method captures the current drive positions and sets the steer
   * angles to form an X pattern (-45°, 45°, 45°, -45° for front-left, front-right, back-left,
   * back-right respectively).
   *
   * <p>Subsequent updates apply position control to hold all modules at their target positions.
   *
  ...
  public void update(double deltaTimeSeconds) {
    if (targetPositions == null) {
      ...
      for (int i = 0; i < targetPositions.length; i++) {
        ...
        targetPositions[i] =
            SwerveKinematicsUtil.optimizeRelativePosition(
                targetPositions[i], targetPositions[i].angle.getMeasure());
      }
    }
