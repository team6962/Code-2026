...
/**
 * Represents a single swerve module, which controls a wheel using of a drive
 * motor, a steer motor, and an absolute encoder on the steer axis.
 * 
 * <p>This class coordinates the {@link DriveMechanism} and {@link SteerMechanism}
 * components, providing a unified interface for:
 * <ul>
 *   <li>Reading module state (velocity and angle)</li>
 *   <li>Reading module position (distance traveled and angle)</li>
 *   <li>Applying control requests to both motors</li>
 *   <li>Logging telemetry data</li>
 *   <li>Updating status signals</li>
 * </ul>
 * 
 * <p>Each swerve module is associated with a {@link Corner} (front-left, front-right,
 * back-left, or back-right) which determines its position on the robot, its
 * configuration from the drivetrain constants, and the index in arrays that
 * data associated with the module is stored in.
 * 
 * @see DriveMechanism
 * @see SteerMechanism
 * @see Corner
...
public class SwerveModule implements SwerveComponent, AutoCloseable {
    ...
    /**
     * Gets the status signals used by this swerve module. These signals will be
     * periodically refreshed by the containing class.
     * 
    ...
    /**
     * Sets the control requests for the drive and steer motors. This is the
     * intended method for motions to command the module to achieve a desired
     * state.
     * 
