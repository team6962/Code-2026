...
/**
 * Simulates a swerve module's steer mechanism using WPILib's DCMotorSim physics simulation.
 *
 * <p>This class bridges between CTRE's TalonFX and CANcoder simulation states and WPILib's physics
 * simulation. It reads the motor controller's applied voltage, runs the physics simulation, and
 * updates both the motor controller's and encoder's simulated sensor values.
 *
 ...
 *
 * <p>Getter methods return the module's angular position, velocity, and acceleration in mechanism
 * (output shaft) coordinates.
 *
 * <p>The {@link #update(double)} method should be called periodically (typically every simulation
 * tick) to advance the physics simulation and synchronize state with the motor controller and
 * encoder.
 */
public class SteerMechanismSim {
  ...
  public SteerMechanismSim(
      Corner corner, DrivetrainConstants constants, TalonFX motorController, CANcoder encoder) {
    this.corner = corner;
  ...
  public void update(double deltaTimeSeconds) {
    ...
    double motorSign =
        constants.SteerMotor.DeviceConfiguration.MotorOutput.Inverted
                == InvertedValue.Clockwise_Positive
            ? -1.0
            : 1.0;
    double encoderSign =
        constants.SteerEncoder.DeviceConfiguration.MagnetSensor.SensorDirection
    ...
    motorControllerSimulation.setRawRotorPosition(
        physicsSimulation
            .getAngularPosition()
            .minus(offset)
            .times(constants.SteerMotor.GearReduction * motorSign));
    ...
    encoderSimulation.setRawPosition(
        physicsSimulation.getAngularPosition().minus(offset).times(encoderSign));
