...
/**
 * Simulates a complete swerve drivetrain including all modules and the
 * gyroscope.
 *
 * <p>This is the top-level simulation class that orchestrates simulation of
 * the entire swerve drive system. It creates and manages
 * {@link SwerveModuleSim} instances for each module and a
 * {@link GyroscopeSim} that derives heading from module motion.
 *
 * <p>Getter methods provide access to the module and gyroscope simulations
 * for inspection or direct manipulation during testing.
 *
 * <p>The {@link #update(double)} method should be called periodically
 * (typically every simulation tick) to advance all subsystem simulations in
 * the correct order: modules first, then gyroscope (which depends on module
 * positions).
 */
