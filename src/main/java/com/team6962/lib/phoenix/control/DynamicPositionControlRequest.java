...
/**
 * Configuration for a control request that can be used to control the position of a motor with
 * dynamic constraints. While the Phoenix 6 library has control requests for each possible
 * combination of motion profile types and output types, this class combines them into a single
 * object with enums to select the desired types. This allows dynamic position control requests of
 * different classes to be created based on two configuration constants.
 *
 * <p>A {@link DynamicPositionControlRequest} cannot be directly applied to a motor controller; it
 * must first be converted to a {@link ControlRequest} object using the {@link #toControlRequest()}
 * method.
 */
public class DynamicPositionControlRequest {
  ...
  public DynamicPositionMotionProfileType MotionProfileType =
      DynamicPositionMotionProfileType.Exponential;
  ...
  /**
   * Constructs a control request with the given target position and trapezoidal constraints.
   *
   * @param Position The position to drive toward in rotations.
   * @param Velocity The maximum velocity constraint in rotations per second.
   * @param Acceleration The maximum acceleration constraint in rotations per second squared.
   * @param Jerk The maximum jerk constraint in rotations per second cubed.
   */
  public DynamicPositionControlRequest(
  ...
  /**
   * Constructs a generic dynamic position control request from a {@link ControlRequest} instance.
   *
   ...
   * @return This control request object, for chaining.
   */
  public DynamicPositionControlRequest withJerk(
   ...
   * @return This control request object, for chaining.
   */
  public DynamicPositionControlRequest withMotionProfileType(
      DynamicPositionMotionProfileType motionProfileType) {
  ...
  /**
   * Returns whether the provided {@link ControlRequest} is a position control request with dynamic
   * constraints.
   *
   * @param controlRequest the control request to check
   * @return true if the control request is a position control request with dynamic constraints,
   *     false otherwise
   */
