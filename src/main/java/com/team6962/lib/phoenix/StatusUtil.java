 ...
 * class provides two methods for logging error codes:
 *
 * <ul>
 ...
 * </ul>
 *
 * Additionally, this class provides the method {@link #toFPGATimestamp(double)} for converting
 * status signal timestamps to FPGA timestamps.
 */
public final class StatusUtil {
  private StatusUtil() {}

  /**
   * Checks the given {@link StatusCode} and, if it is not OK, prints an error message and stack
   * trace.
   *
  ...
  public static void check(StatusCode code) {
    ...
    System.err.println(
        "Status Code " + code.value + " " + code.getName() + ": " + code.getDescription());
  ...
  /**
   * Unwraps the value from the given {@link StatusSignal}, checking the status code and logging an
   * error with a stack trace if it is not OK.
   *
  ...
  /**
   * Converts a status signal timestamp to an FPGA timestamp in the same timebase as return values
   * of {@link Timer#getFPGATimestamp()}.
   *
