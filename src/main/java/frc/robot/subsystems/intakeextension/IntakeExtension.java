                ...
                () -> {
                  motor.setControl(
                      new DynamicMotionMagicVoltage(
                          IntakeExtensionConstants.AGITATED_POSITION.in(Meters), 0.5, 1.0));
                },
                () -> {
                  if (!getPosition()
                      .isNear(
                          ...
                          IntakeExtensionConstants.POSITION_TOLERANCE)) {
                    motor.setControl(
                        new DynamicMotionMagicVoltage(getPosition().in(Meters), 0.5, 1.0));
                  }
                })
            .until(
                () ->
  ...
  public Command extendSlow() {
    return startEnd(
            () -> {
              motor.setControl(
                  new DynamicMotionMagicVoltage(
                      IntakeExtensionConstants.EXTEND_POSITION.in(Meters), 0.5, 1.0));
            },
            () -> {
              if (!getPosition()
                  .isNear(
                      IntakeExtensionConstants.EXTEND_POSITION,
                      IntakeExtensionConstants.POSITION_TOLERANCE)) {
                motor.setControl(new DynamicMotionMagicVoltage(getPosition().in(Meters), 0.5, 1.0));
              } else {
