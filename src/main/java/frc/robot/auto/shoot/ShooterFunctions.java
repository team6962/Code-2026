...
/**
 * A set of functions that can be used to calculate shooting parameters (hood angle, flywheel
 * velocity, etc) based on the current state of the robot and the target.
 *
 * <p>These functions are generated from a CSV file containing empirical data of the shooter's
 * performance at various distances and angles. The CSV file should have the following columns:
 * distance (in inches), hood angle (in degrees), flywheel velocity (in rotations per second), and
 * flight time (in seconds). The class uses spline interpolation to create continuous functions from
 * the discrete data points in the CSV file.
 */
public class ShooterFunctions {
  ...
  /**
   * Calculates the hood angle required to score given a distance to the target. Clamps the output
   * to the maximum and minimum hood angles if the input distance is outside the valid range.
   *
  ...
  public Angle getHoodAngle(Distance distance) {
    return Degrees.of(
        hoodAngleFunction.value(MeasureUtil.clamp(distance, minDistance, maxDistance).in(Inches)));
  }

  /**
   * Predicts the distance to the target based on the given hood angle, assuming that the flywheel
   * is at the right speed. Clamps the input hood angle to the maximum and minimum hood angles if it
  ...
  public Distance getDistance(Angle hoodAngle) {
    return Inches.of(
        distanceFunction.value(
            MeasureUtil.clamp(hoodAngle, minHoodAngle, maxHoodAngle).in(Degrees)));
  }
  ...
  /**
   * Calculates the flight time for a shot given the current distance to the target. Clamps the
   * input distance to the maximum and minimum valid distances if it is outside the valid range.
   *
   * @param distance the distance to the target
  ...
  public Time getFlightTime(Distance distance) {
    return Seconds.of(
        flightTimeFunction.value(MeasureUtil.clamp(distance, minDistance, maxDistance).in(Inches)));
  }
   ...
   * @return true if the distance is greater than or equal to minDistance and less than or equal to
   *     maxDistance
   */
